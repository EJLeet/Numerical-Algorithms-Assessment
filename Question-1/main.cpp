...
{ return T(-0.1) * x * x * x * x - T(0.15) * x * x * x - T(0.5) 
         * x * x - T(0.15) * x + T(1.2); }
...
int main()
{
    ...
    float flt_h_pred = predict_h(float(0.00001), float(0.1), 
                                 float(0.00001), "float.txt");
    half_float::half hlf_h_pred(predict_h(half_float::half(0.001), 
                                          half_float::half(0.5),
                                          half_float::half(0.001), 
                                          "half.txt"));

    // Find theoretical H based on machine epsilon
    // Use double to increase  precision of result using <TYPE> epsilon
    double dbl_h_opt = cbrt((double(3) * __DBL_EPSILON__) / 
                       abs(third_derivative(double(0.5))));
    double flt_h_opt = cbrt((double(3) * __FLT_EPSILON__) / 
                       abs(third_derivative(double(0.5))));
    double hlf_h_opt = cbrt((double(3) * __FLT16_EPSILON__ / 
                       abs(third_derivative(double(0.5)))));
    ...
    /*
        ...
        Double = y=0.357730803912x^2 + -0.000000929785x + 0.000000000023 
                                                     = h opt = 0.00000128751
    ...
    cout << "Theoretical Optimal H Using Double Machine Epsilon = \t\t" << dbl_h_opt
         << endl
         << "Theoretical Optimal H Using Float Machine Epsilon = \t\t"
         << flt_h_opt << endl
         << "Theoretical Optimal H Using Half Machine Epsilon = \t\t"
         << hlf_h_opt << endl
         << endl;
    ...
    cout << "Central Difference Derivative Using Theoretical H as Double = \t"
         ...
         << central_dif(double(0.5), hlf_h_opt) << endl
         << endl;
