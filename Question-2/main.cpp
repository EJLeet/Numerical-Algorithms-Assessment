...
{ return (fn(x + h, t) - fn(x - h, t)) / (2 * h); }
double fn_2(double x, double h, int t) // function to evaluate f"
{ return (fn(x + h, t) - 2 * fn(x, t) + fn(x - h, t)) / (h * h); }
...
int main()
{
    ...
    for (int i = 1; i <= 30; i++) // work out dv^2/dt
        outfile << fn_2(i, 0.25, i) << endl;
